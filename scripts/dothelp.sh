...
main() {
    ...
    section "dotaudit"
    cmd "dotaudit [--only git|policy|privacy|disk] [--github] [--no-report]"
    desc "Read-only audit of every git repo under ~/dev"
    desc "Usage: dotaudit"
    desc "       dotaudit --github        (also fetch public/private from GitHub)"
    desc "       dotaudit --only privacy  (one category only)"
    desc "When: Periodically, or before pushing anything public"
    desc "Does: Reports unpushed work, repos with no remote, missing LICENSE/"
    desc "      .gitignore, secrets and personal data in tracked files, and"
    desc "      large/rebuildable dirs. Changes nothing - there is no --fix."
    desc "Note: Report goes to ~/dev/audit-reports/, never into this repo."
    desc "      See docs/dev-audit.md"
    desc "Test: ./test.sh dotaudit   (or ./test.sh for every suite)"
